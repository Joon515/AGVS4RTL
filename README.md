...
AGVS4RTL 是一个基于 LLM 的 HDL 生成与验证系统。本次开发补齐了预处理数据结构与 Rich TUI 仪表盘，并增加了 API Key 加密配置能力。
...
- 预处理与数据结构：app/pre_agent/structure_request.py
- TUI 仪表盘：app/ui/tui.py
- 配置加密：app/ui/config_store.py
- TUI 测试与演示：app/dev_tests/run_tui.py、app/dev_tests/run_tui_components.py、app/ui/demo_tui.py
...
- 开发测试脚本目录：app/dev_tests/
- 运行测试：python3 app/dev_tests/run_tui.py
- 演示仪表盘：python3 app/ui/demo_tui.py

## 开发者提醒：依赖变更与镜像重建

用户使用 `setup.py` 时不会感知依赖问题，但开发者修改 `requirements.txt` 或 `requirements-dev.txt` 后必须触发镜像构建，否则容器仍会沿用旧依赖。
