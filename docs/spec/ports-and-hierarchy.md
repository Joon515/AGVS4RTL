...
> 版本说明：当前版本暂不实现图结构测试与非静态测试，以下内容作为下一版本实现目标。下一版本实现时，结构化输入与校验上下文依旧采用 AST 结果，不使用 Regex 进行端口/层次语义解析。

- ✅ **循环依赖检测**：模块实例化关系图必须是有向无环图（DAG）
- ✅ **层次深度检查**：统计最大层次深度并与项目阈值比对
- ✅ **孤立模块检查**：除顶层外，不应存在未被任何父模块引用的模块
- ✅ **引用完整性检查**：`children` 中出现的模块名必须在 `hierarchy.modules` 中可解析

### AST 提取与适用边界

- **适用场景**：从已有 Verilog/SystemVerilog 反向提取模块、端口、参数、实例关系，作为架构生成输入。
- **推荐用途**：
  - 作为架构增量设计的 `base_architecture`
  - 为 RAG 建库提取结构化特征（接口类型、层次深度、参数化标记）
- **非适用场景**：架构 JSON 的一致性校验应使用图论算法，不应强依赖 AST。
- **Regex 保留场景**：仅用于工具日志/告警文本提取，不用于端口和模块层次语义解析。
