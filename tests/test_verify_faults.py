...
@pytest.mark.docker
def test_verify_pass(docker_services):
    case_name, expected_verdict, mutate = CASES[0]
...
@pytest.mark.docker
def test_verify_missing_port(docker_services):
    case_name, expected_verdict, mutate = CASES[1]
    task_id, task_dir, spec_path, rtl_path = _prepare_case(case_name, mutate)
    data = _call_verify(task_id, task_dir, spec_path, rtl_path)
    report = data["report"]
    actual_verdict = report["verdict"]
...
@pytest.mark.docker
def test_verify_direction_mismatch(docker_services):
    case_name, expected_verdict, mutate = CASES[2]
    task_id, task_dir, spec_path, rtl_path = _prepare_case(case_name, mutate)
    data = _call_verify(task_id, task_dir, spec_path, rtl_path)
    report = data["report"]
    actual_verdict = report["verdict"]
...
@pytest.mark.docker
def test_verify_width_mismatch(docker_services):
    case_name, expected_verdict, mutate = CASES[3]
    task_id, task_dir, spec_path, rtl_path = _prepare_case(case_name, mutate)
    data = _call_verify(task_id, task_dir, spec_path, rtl_path)
    report = data["report"]
    actual_verdict = report["verdict"]
...
@pytest.mark.docker
def test_verify_extra_port(docker_services):
    case_name, expected_verdict, mutate = CASES[4]
    task_id, task_dir, spec_path, rtl_path = _prepare_case(case_name, mutate)
    data = _call_verify(task_id, task_dir, spec_path, rtl_path)
    report = data["report"]
    actual_verdict = report["verdict"]
...
@pytest.mark.docker
def test_verify_compile_error(docker_services):
    case_name, expected_verdict, mutate = CASES[5]
    task_id, task_dir, spec_path, rtl_path = _prepare_case(case_name, mutate)
    data = _call_verify(task_id, task_dir, spec_path, rtl_path)
    report = data["report"]
    actual_verdict = report["verdict"]
