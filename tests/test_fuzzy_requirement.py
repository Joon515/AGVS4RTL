...
def _assert_fuzzy_requirement_success(result):
    ...
    if {"i_clk", "i_rst_n", "o_done"}.issubset(port_names):
        print(f"⚠️  Architect fell back to default dummy sequential ports: {ports}")
        print("⚠️  LLM quality is non-deterministic — this is expected variance, not a bug")
    else:
...
@pytest.mark.llm
@pytest.mark.slow
def test_fuzzy_requirement(parser_url, api_client):
    payload = {
